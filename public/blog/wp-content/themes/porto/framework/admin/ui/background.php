...
?>
    <div <?php echo build_section_class( 'section-background', $item ); ?>>
        ...
        <div class="controls">
            ...
            <?php if ( isset( $item['use_pattern'] ) ) { ?>
            <div class="section-radio-img section-pattern">
                <h3 class="heading"><?php _e( 'Background Patterns', 'spyropress' ); ?></h3>
                <ul id="bg_patterns">
                <?php
                    foreach ( spyropress_panel_background_pattern() as $path => $label ) {
                        printf('
                            <li><label class="radio-img%6$s" for="%1$s">
                                <input type="radio" id="%1$s" name="%3$s[background-pattern]" value="%2$s" %5$s />
                                <img src="%4$s">
                            </label></li>',
                            $item['name'].'_'.$label, $path, $item['name'], str_replace($label, $label.'_thumb', $path), checked( $value['background-pattern'], $path, false ), ($value['background-pattern'] == $path) ? ' selected': ''
                        );
                    }
                ?>
                </ul>
 ...
 * Background Patterns
 */
function spyropress_panel_background_pattern() {
    $path = assets_img(). 'patterns/';
    
    return apply_filters( 'spyropress_background_patterns', array(
        $path.'45degreee_fabric.png' => '45degreee_fabric',
        $path.'60degree_gray.png' => '60degree_gray',
        $path.'always_grey.png' => 'always_grey',
        $path.'argyle.png' => 'argyle',
        $path.'batthern.png' => 'batthern',
        $path.'beige_paper.png' => 'beige_paper',
        $path.'bgnoise_lg.png' => 'bgnoise_lg',
        $path.'black_denim.png' => 'black_denim',
        $path.'black_linen_v2.png' => 'black_linen_v2',
        $path.'black_paper.png' => 'black_paper'
    ));
}
?>
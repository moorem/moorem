...
builder.rows = {
    ...
    init: function() {
        ...
        $( '#builder' ).on( 'click', '.builder-row-delete', function( event ) {
            ...
            $( '#builder-delete-row-confirm', b.dialogs.delete ).click(function() {
                if ( !r.removing ) {
                    r.removing = true;
        ...
        $( 'form.builder-module-edit-form' ).live( 'submit', function( event ) {
            event.preventDefault();
            b.modules.submit_module( $(this), {}, 'submit-module-response' );
        });
...
builder.modules = {
    ...
    save_rich_editor: function( event, data ) {
        ...
        target_id = $( '.builder-rich-text', '#' + data.form_id ).attr('id');
        if( target_id ) {
            tiny_ed = tinyMCE.get( target_id );
            tiny_ed.save();
            tinyMCE.remove( tiny_ed );

...
class SpyropressPricingTable extends SpyropressComponent {
    ...
    function render_table( $slider_id ) {
        ...
        foreach( $columns as $column ) {
            ...
            $class = ( 4 == $count ) ? 'span3' : 'span4';
            $plan_class = 'plan';
            $popular = '';
            if( isset( $column['recommended'] ) ) {
                $class .= ' center';
                $plan_class .= ' most-popular';
                $popular = '<div class="plan-ribbon-wrapper"><div class="plan-ribbon">Popular</div></div>';
            }
            ...
            <div class="' . $class . '">
				<div class="' . $plan_class . '">
					...
					<a class="btn btn-large btn-primary" href="' . $column['url'] . '">' . $btn_text . '</a>
					<ul>' . $features . '</ul>
				</div>
			</div>';
        }

...
class Bootstrapwp_Walker_Nav_Menu extends Walker_Nav_Menu {
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
        ...
        if( is_str_contain( 'phone', $item->attr_title ) ) {
            
            $output .= $indent . '<li class="phone"><span><i class="icon-phone"></i>' . $item->title . '</span></li>';
        }
        elseif( is_str_contain( 'divider', $item->title ) ) {
            ...
            $attributes .= ( !is_str_contain( '?', $item->url ) && is_str_contain( '#', $item->url ) ) ? ' data-hash' : '';
